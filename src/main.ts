...
<div class="shell">
  <header><a class="brand" href="/" aria-label="Kikitori — trang đầu"><span class="seal" lang="ja">聞</span><span>KIKITORI <small lang="ja">聞き取り</small></span></a><span class="edition">聴解ノート <span> / 01</span></span></header>
  <main>
    <section class="intro"><p class="eyebrow">JAPANESE LISTENING NOTEBOOK</p><h1 lang="ja">一文ずつ、<br class="mobile-break">聞いてみよう。</h1><p>Nghe, đọc, hiểu. Từng câu một.</p></section>
    ...
    <section id="workspace" hidden aria-label="Bài nghe hiện tại">
      ...
      <div class="process-row"><div><button id="transcribe" class="primary">Tạo transcript & bản dịch</button><button id="cancel" class="text-button" hidden>Dừng xử lý</button></div><p>AI chạy trên máy bạn. Lần đầu cần tải khoảng 1 GB;<br>nên dùng máy tính và giữ tab mở trong lúc xử lý.</p></div>
    </section>
    ...
    <section class="transcript-section" aria-labelledby="transcript-title"><div class="section-label transcript-heading"><h2 id="transcript-title">02 <span>Bản nghe</span><small id="sentence-count"></small></h2><fieldset class="furigana"><legend>Furigana</legend><label><input type="radio" name="furigana" value="hover" checked/><span>Hover</span></label><label><input type="radio" name="furigana" value="always"/><span>Always</span></label></fieldset></div>
      ...
      <p id="transcript-note" class="transcript-note" hidden>Chạm hoặc nhấn Enter vào câu để nghe từ đó. Furigana hiện khi di chuột hoặc lấy nét; trên điện thoại có thể chọn Always. Mốc có dấu ≈ là thời gian ước lượng khi tách câu. AI có thể nghe, dịch hoặc đọc tên riêng sai.</p>
    </section>
  </main>
...
async function renderSentences(run:number){
  ...
  for(const sentence of sentences){
    const row=document.createElement('button');row.className='sentence';row.type='button';
    ...
    const translation=document.createElement('span');translation.className='translation';translation.textContent='Đang chờ bản dịch…';
    content.append(japanese,translation);row.append(time,content);
    row.addEventListener('click',async()=>{audio.currentTime=sentence.start;highlight();try{await audio.play();}catch{error('Nhấn nút phát để bắt đầu nghe.');}});
...
transcribe.addEventListener('click',async()=>{
  ...
  try{
    ...
    worker.onmessage=event=>{
      ...
      if(data.type==='translation'){const sentence=sentences[data.id];if(sentence)sentence.translation=data.text;const el=rows.get(data.id)?.querySelector('.translation');if(el)el.textContent=data.text;}
